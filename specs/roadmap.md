...
- [x] **§4 compliance — field-driven additions (2026-07-05, `309d780`)** — pack grew **46 → 48**: **STATUS-010** (MeterValues must be preceded by `StatusNotification(Charging)`) + **STATUS-011** (same fault must report a consistent `errorCode`), both Major/heuristic, from field log `data/samples/DC060 …`. Plus 2 Status-section UI fixes: Error-Code-Frequency **Info+Status** columns; **Load & Analyze** processing spinner. +6 tests (332). The `docs/business_case_compliance_check.md` ledger grows as field cases are shared (1 row + 1 rule + 1 sample per case).
