...
- [x] **Phase 6 — Validation Engine integration** (2026-06-22): new **Type-Aware Validation (L1–L3)** section (#20, after WebSocket Health) — `render/sections/validation.ts`. On-demand button lazy-loads `src/services/validation` (code-split, typed-ocpp ~509 kB chunk; main bundle stays 200 kB) and runs `validateBatch(framesFromMessages(r.messages))`, rendering the `ValidationReport` (cards + violations + unmatched exchanges; textContent per the engine /cso XSS note). Adapter is a faithful pass-through (`OcppRawMessage ≡ RawFrame`). +7 tests (261). Arch `docs/superpowers/specs/2026-06-22-parser-phase6-validation-integration-arch.md`. **Skill chain:** Think (TYPEVALIDATION.md) + Plan (`/plan-eng-review`) ✅; **Review/Test (`/review`+`/cso`, `/qa`) still pending.** L4 = engine Phase 2 (stub). (consumption = direct monorepo import, decided `knowledge/decisions/2026-06-21-validation-engine-consumption-model.md`): merge `feat/validation-engine` → `main`, then onto the integration line (or merge engine branch into parser branch). Blocks Phase 6 start.
