...
- **Next:** `/review`+`/cso`+`/qa` on **Phase 6** (validation engine — still pending); fix the cross-file id-collision bug; then deploy decision (Help modal + parked items + `vite base` for Pages). **CMS · Charger Emulator · Training Emulator** — planned.
